...
| $R$ | максимальный ID в распознанном входе до очистки плюс 1; длина адресного пространства вершин, включая промежутки |
...
$L_1/N \lt \varepsilon$ нельзя вывести строгую абсолютную ошибку; поэтому ниже
...
проверка строже и выполняется для каждого блока; $m_{s,d}$ ниже — число рёбер
блока до дедупликации:
